    ...
    for(Int_t i_ev=0; i_ev<nEvent; i_ev++) {
      ...
      for(Int_t i_mu=0; i_mu<event.nMuon; i_mu++)
      {
        ...
        for(Int_t j_mu=i_mu+1; j_mu<event.nMuon; j_mu++) // -- starts from i_mu+1: no duplication -- //
        {
          ...
          if( tnpPair_ij->IsValid() )  { tnpPairs.push_back( tnpPair_ij ); }
          else delete tnpPair_ij;

          TnPPair *tnpPair_ji = new TnPPair( mu_ith, mu_jth, ntuple );
          if( tnpPair_ji->IsValid() )  { tnpPairs.push_back( tnpPair_ji ); }
          else delete tnpPair_ji;
        }
      }

      for( const auto& tnpPair : tnpPairs )
        tnpHist->Fill( tnpPair, weight );

      for( auto tnpPair : tnpPairs )
        delete tnpPair;
